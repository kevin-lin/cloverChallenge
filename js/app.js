...
.controller('indexController', ['$scope', '$location', '$window', '$http', function($scope, $location, $window, $http) {
    var APP_ID = 'WXSGVB6YKHQKG';
    $scope.urlParams = $location.search();
    var urlParams = $location.search();
    if(typeof urlParams.merchant_id === 'undefined'){
        $window.location.href = 'https://www.clover.com/oauth/authorize?client_id=' + APP_ID + '&response_type=token';
    }
    ...
    $scope.getReceipt = function(orderId){
        ...
        $http({
            ...
            url: 'https://api.clover.com/v3/merchants/' + urlParams.merchant_id + '/orders/' + orderId + '/line_items'
        }).then(function successCallback(response) {
